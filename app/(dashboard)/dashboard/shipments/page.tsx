...
const config = {
  ...
  actions: [
    {
      ...
      onClick: async (item: any) => {
        await adminApi.markShipmentShipped(item.id, {
          tracking_number: `TRACK-${item.id}`,
        });
      },
    },
    {
      ...
      tone: 'secondary',
      onClick: async (item: any) => {
        await adminApi.markShipmentInTransit(item.id);
      },
    },
    {
      ...
      tone: 'primary',
      onClick: async (item: any) => {
        await adminApi.markShipmentDelivered(item.id);
      },
    },
  ],
  readOnly: false,
  fields: [
    ...
    { name: 'address', label: 'Address ID', type: 'number' },
    { name: 'shipping_method', label: 'Shipping method ID', type: 'number' },
  ],
};
